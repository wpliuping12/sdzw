	...
	function selectPicTitle($sql) {
	   $res = $this->query ( $sql );
		if ($res !== false) {
			...
			while ( $row = mysql_fetch_assoc ( $res ) ) {
			    $preg = "/ src=\"(.+?)\".*?>/";
			    preg_match_all($preg,$row["newsContent"],$new_cnt,1 );
				$arr [] = $row;
				$arr [] = array("src"=>$new_cnt[0][0]);
// 				if(count($new_cnt)>0)
// 				  $arrimg[] = array("srcimg"=>$new_cnt[1][0]);
// 				else 
// 				    $arrimg[] = array("srcimg"=>"");
			}
// 			$result = array_merge_recursive($arr, $arrimg);
			return $arr;
		} else {
			return false;
		}
	 ...
	 * @return int
	 */
	function getRowsNum1($sql) {
	    $res = $this->query ( $sql );
	    $aa = mysql_fetch_row ( $res );
	    return $aa[0];
	}
	/**
	 * 返回查询的结果的第一个数据
